...
(function(window,document,undefined){'use strict';var _objectToParametersString=function(data){return Object.keys(data).map(function(key){var value=data[key];if(typeof value==='object'){value=JSON.stringify(value);}
...
return output;},ajaxLoading=false,ajaxURL=wpgdprcData.ajaxURL,ajaxSecurity=wpgdprcData.ajaxSecurity,_ajax=function(data,values,$form,delay){var $feedback=$form.querySelector('.wpgdprc-feedback'),value=values.slice(0,1);if(value.length>0){var $row=$form.querySelector('tr[data-id="'+value[0]+'"]');$row.classList.remove('wpgdprc-status--error');$row.classList.add('wpgdprc-status--processing');$feedback.setAttribute('style','display: none;');$feedback.classList.remove('wpgdprc-feedback--error');$feedback.innerHTML='';setTimeout(function(){var request=new XMLHttpRequest();data.data.value=value[0];request.open('POST',ajaxURL);request.setRequestHeader('Content-type','application/x-www-form-urlencoded; charset=UTF-8');request.send(_objectToParametersString(data));request.addEventListener('load',function(){if(request.response){var response=JSON.parse(request.response);$row.classList.remove('wpgdprc-status--processing');if(response.error){$row.classList.add('wpgdprc-status--error');$feedback.innerHTML=response.error;$feedback.classList.add('wpgdprc-feedback--error');$feedback.removeAttribute('style');}else{values.splice(0,1);$row.querySelector('input[type="checkbox"]').remove();$row.classList.add('wpgdprc-status--removed');_ajax(data,values,$form,500);}}});},(delay||0));}};document.addEventListener('DOMContentLoaded',function(){var $formAccessRequest=document.querySelector('.wpgdprc-form--access-request'),$formDeleteRequest=document.querySelectorAll('.wpgdprc-form--delete-request');if($formAccessRequest!==null){var $feedback=$formAccessRequest.querySelector('.wpgdprc-feedback'),$emailAddress=$formAccessRequest.querySelector('#wpgdprc-form__email'),$consent=$formAccessRequest.querySelector('#wpgdprc-form__consent');$formAccessRequest.addEventListener('submit',function(e){e.preventDefault();if(!ajaxLoading){ajaxLoading=true;$feedback.style.display='none';$feedback.classList.remove('wpgdprc-feedback--success','wpgdprc-feedback--error');$feedback.innerHTML='';var data={action:'wpgdprc_process_action',security:ajaxSecurity,data:{type:'access_request',email:$emailAddress.value,consent:$consent.checked}},request=new XMLHttpRequest();data=_objectToParametersString(data);request.open('POST',ajaxURL,true);request.setRequestHeader('Content-type','application/x-www-form-urlencoded; charset=UTF-8');request.send(data);request.addEventListener('load',function(){if(request.response){var response=JSON.parse(request.response);if(response.message){$formAccessRequest.reset();$emailAddress.blur();$feedback.innerHTML=response.message;$feedback.classList.add('wpgdprc-feedback--success');$feedback.removeAttribute('style');}
if(response.error){$emailAddress.focus();$feedback.innerHTML=response.error;$feedback.classList.add('wpgdprc-feedback--error');$feedback.removeAttribute('style');}}
ajaxLoading=false;});}});}
if($formDeleteRequest.length>0){$formDeleteRequest.forEach(function($form){var $selectAll=$form.querySelector('.wpgdprc-select-all');$form.addEventListener('submit',function(e){e.preventDefault();var $this=e.target,$checkboxes=$this.querySelectorAll('.wpgdprc-checkbox'),data={action:'wpgdprc_process_action',security:ajaxSecurity,data:{type:'delete_request',session:wpgdprcData.session,settings:JSON.parse($this.dataset.wpgdprc)}};$selectAll.checked=false;_ajax(data,_getValuesByCheckedBoxes($checkboxes),$this);});if($selectAll!==null){$selectAll.addEventListener('change',function(e){var $this=e.target,checked=$this.checked,$checkboxes=$form.querySelectorAll('.wpgdprc-checkbox');$checkboxes.forEach(function(e){e.checked=checked;});});}});}});})(window,document);
...
$=jQuery;$(document).ready(function(){$('#place_order').unbind().click(function(e){e.preventDefault();if(($('#billing_first_name').val()=='')||($('#billing_last_name').val()=='')||($('#billing_email').val()=='')){alert('Please make sure you have filled out first name, last name, and email to receive your order.');}else if(($('input[name="ss_wc_mailchimp_opt_in"]:checked').length<1)&&(localized_sexy_config.total==0)){let result=confirm("Please consider allowing me to email you my new new music and show announcements. It's the only way I can continue giving away free music! Please click OK to accept!");if(result){$('input[name="ss_wc_mailchimp_opt_in"]').prop('checked',true);$('form[name="checkout"]').submit();}else{$('form[name="checkout"]').submit();}}else{$('form[name="checkout"]').submit();}});$('#powered').css('display','none');$('.home-subscribe-widget').hide();$('.filter-drop-spacer').remove();$('.close').unbind().click(function(e){e.preventDefault();var today=new Date();var nextweek=new Date(today.getFullYear(),today.getMonth(),today.getDate()+7);document.cookie='disableLandingPagePopUpNew=1; '+'expires='+nextweek+';'+'path=/';$('.home-subscribe-widget').fadeOut('fast');});if($(window).width()<768){$('.featured_image_wrapper').css({'background-attachment':'scroll'});$('.home-top').css({'background-attachment':'scroll'});$('.home-page-2').css({'background-attachment':'scroll'});}else{if(!readCookie('disableLandingPagePopUpNew')){$('.home-subscribe-widget').fadeIn('fast');}
$('.menu-item a').unbind().click(function(e){if($(this).attr('href')=='#'){e.preventDefault();window.location=$(this).parent().find('ul li a').first().attr('href');}});var fullPath=location.pathname+location.search+location.hash;if(fullPath==="/"){$('.site-header').append('<span class="home-down"><i class="fa fa-angle-down"></i></span>');var timer;$(document).mousemove(function(){if(timer){clearTimeout(timer);timer=0;}
$('.home-down .fa-angle-down').fadeIn('slow');timer=setTimeout(function(){$('.home-down .fa-angle-down').fadeOut()},2000)});}}});if($('p').hasClass('confirmed')){$('.instructions').hide();}
