...
$=jQuery;$(document).ready(function(){let homeVideo=$('#landing_yt_player').data('id');let homeVideoStartTime=$('#landing_yt_player').data('start-time');let thumb='<img src="https://img.youtube.com/vi/'+homeVideo+'/maxresdefault.jpg">';let thumbSrc='https://img.youtube.com/vi/'+homeVideo+'/maxresdefault.jpg';if(typeof homeVideo!=='undefined'){if($(window).width()<768){$('body').prepend('<div class="video-background" style="background-image: url('+thumbSrc+')">'+'<div class="video-foreground" >'+'</div>'+'</div>');}else{let fullPath=location.pathname+location.search+location.hash;$('body').prepend('<div class="video-background">'+'<div class="video-foreground">'+'<iframe class="home_video" id="yt_home_embed" width="1280" height="720" src="https://www.youtube.com/embed/'+homeVideo+'?rel=0&controls=0&showinfo=0&autoplay=1&loop=1&enablejsapi=1&playlist=79EUORqIh0c&start='+homeVideoStartTime+'" frameborder="0" allowfullscreen volume="0"></iframe>'+'</div>'+'</div>');let tag=document.createElement('script');tag.src="https://www.youtube.com/iframe_api";let firstScriptTag=document.getElementsByTagName('script')[0];firstScriptTag.parentNode.insertBefore(tag,firstScriptTag);let player;let div,n,v=document.getElementsByClassName("youtube-player");for(n=0;n<v.length;n++){div=document.createElement("div");div.setAttribute("data-id",v[n].dataset.id);div.innerHTML=labnolThumb(v[n].dataset.id);div.onclick=labnolIframe;v[n].appendChild(div);}}}});function onYouTubeIframeAPIReady(){player=new YT.Player('yt_home_embed',{events:{'onReady':onPlayerReady}});}
function onPlayerReady(){player.mute();player.playVideo();}
function labnolThumb(id){let homeVideo=$('#landing_yt_player').data('id');let thumb='<img src="https://img.youtube.com/vi/'+homeVideo+'/0.jpg">';play='<div class="play"></div>';return thumb.replace("ID",id)+play;}
function labnolIframe(){let iframe=document.createElement("iframe");let embed="https://www.youtube.com/embed/videoseries?list=PLCcd4NlKH5YzNrrji-f_3elED_tmifwUz";iframe.setAttribute("src",embed);iframe.setAttribute("frameborder","0");iframe.setAttribute("allowfullscreen","1");this.parentNode.replaceChild(iframe,this);};
...
instance++;self.data(pluginPfx,{minChars:(self.data('min-chars')!==undefined)?self.data('min-chars'):1,showLoader:(self.data('show-loader')!==undefined)?self.data('show-loader'):true,showMore:(self.data('show-more')!==undefined)?self.data('show-more'):true,showPage:(self.data('show-page')!==undefined)?self.data('show-page'):true,useAnalytics:(self.data('use-analytics')!==undefined)?self.data('use-analytics'):false,instance:instance,resultBlock:'#aws-search-result-'+instance});var d=self.data(pluginPfx);if($searchForm.length>0){methods.init.call(this);}
...
$=jQuery;$(document).ready(function(){$('#place_order').unbind().click(function(e){e.preventDefault();if(($('#billing_first_name').val()=='')||($('#billing_last_name').val()=='')||($('#billing_email').val()=='')){alert('Please make sure you have filled out first name, last name, and email to receive your order.');}else if(($('input[name="ss_wc_mailchimp_opt_in"]:checked').length<1)&&(localized_sexy_config.total==0)){let result=confirm("Please consider allowing me to email you my new new music and show announcements. It's the only way I can continue giving away free music! Please click OK to accept!");if(result){$('input[name="ss_wc_mailchimp_opt_in"]').prop('checked',true);$('form[name="checkout"]').submit();}else{$('form[name="checkout"]').submit();}}else{$('form[name="checkout"]').submit();}});$('#powered').css('display','none');$('.home-subscribe-widget').hide();if(!readCookie('disableLandingPagePopUpNew')){$('.home-subscribe-widget').fadeIn('fast');}
$('.filter-drop-spacer').remove();$('.close').unbind().click(function(e){e.preventDefault();var today=new Date();var nextweek=new Date(today.getFullYear(),today.getMonth(),today.getDate()+7);document.cookie='disableLandingPagePopUpNew=1; '+'expires='+nextweek+';'+'path=/';$('.home-subscribe-widget').fadeOut('fast');});if($(window).width()<768){}else{$('.menu-item a').unbind().click(function(e){if($(this).attr('href')=='#'){e.preventDefault();window.location=$(this).parent().find('ul li a').first().attr('href');}});}});if($('p').hasClass('confirmed')){$('.instructions').hide();}
